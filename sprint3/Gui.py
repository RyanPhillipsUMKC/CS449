...
class App(tk.Tk):
    ...
    def on_board_cell_click(self, event) -> None:
        ...
        if new_game_state != GameStateType.Ongoing:
            if new_game_state == GameStateType.Blue_Win:
                messagebox.showinfo("Blue Wins!", message="The blue player has won the game.")
            elif new_game_state == GameStateType.Red_Win:
    ...
    def _update_sos_lines(self, turn):
        ...
        for sos_indexes in soses_indexes:
            ...
            first_spot_center_x = (first_spot_x1 + first_spot_x2) / 2
            first_spot_center_y = (first_spot_y1 + first_spot_y2) / 2

            last_spot_center_x = (last_spot_x1 + last_spot_x2) / 2
            last_spot_center_y = (last_spot_y1 + last_spot_y2) / 2

            self.board_canvas.create_line(
                first_spot_center_x, first_spot_center_y, 
                last_spot_center_x, last_spot_center_y, 
                fill="red" if turn == PlayerType.Red else "blue", width=3)

    def _get_cell_spot_id_from_row_col(self, row, col):
        for id, params in self.canvas_board_cell_index_to_params.items():
            if params.row == row and params.col == col:
                return id
        return -1
